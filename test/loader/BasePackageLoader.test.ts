...
describe('BasePackageLoader', () => {
  ...
  function setupLoadPackage(name?: string, version?: string, loadStatus?: string, tarball?: Readable, packageJsonPath?: string, resourceAtPath?: string, 
                            currentBuildDate?: string, resourceInfo?: object) : any {
    const pkgVars = {
      name : name,
      version: version,
      ...
      resourceAtPath: resourceAtPath? resourceAtPath : {'resource' : `${packageJsonPath}`, 'date': '20240824230227', 'resourceType': 'resourceTypeName'},
      currentBuildDate: currentBuildDate? new Promise<string>((resolve) => resolve(currentBuildDate)) : new Promise<string>((resolve) => resolve('20240824230227')),
  ...
  describe('#loadPackage', () => {
    ...
    it('should use current version if dev version was not found in the cache', async () => {
      ...
      packageCacheMock.isPackageInCache.calledWith(pkg.name, pkg.version).mockReturnValueOnce(false);
      packageCacheMock.getPackageJSONPath.calledWith(pkg.name, 'current').mockReturnValueOnce(pkg.packageJsonPath);
      packageCacheMock.getResourceAtPath.calledWith(pkg.packageJsonPath).mockReturnValueOnce(pkg.resourceAtPath);
      currentBuildClientMock.getCurrentBuildDate.calledWith(pkg.name, undefined).mockReturnValueOnce(pkg.currentBuildDate);
      currentBuildClientMock.downloadCurrentBuild.calledWith(pkg.name, 'current').mockResolvedValue(pkg.tarball);
      const loadPackageFromCacheSpy = jest
        .spyOn(BasePackageLoader.prototype as any, 'loadPackageFromCache')
        .mockReturnValueOnce({ name: pkg.name, version: pkg.version, resourceCount: 5 });

      const result = await loader.loadPackage(pkg.name, pkg.version);
      expect(loader.getPackageLoadStatus).toHaveBeenCalledWith(pkg.name, pkg.version);
      expect(loggerSpy.getMessageAtIndex(-2, 'info')).toBe('Falling back to some.ig#current since some.ig#dev is not locally cached. To avoid this, add some.ig#dev to your local FHIR cache by building it locally with the HL7 FHIR IG Publisher.');
    ...
    it('should successfully download current version to cache that is missing or stale', async () => {
      ...
      expect(loggerSpy.getMessageAtIndex(-1, 'debug')).toBe('Cached package date for some.ig#current (2024-08-24T23:02:27) matches last build date (2024-08-24T23:02:27), so the cached package will be used');
      expect(loggerSpy.getLastMessage('info')).toBe('Loaded some.ig#current with 5 resources');
      expect(loadPackageFromCacheSpy).toHaveBeenCalledWith('some.ig', 'current');
      expect(result).toBe(LoadStatus.LOADED);
    });
    ...
    it('should not successfully download current version to cache that is missing or stale', async () => {
      const pkg = setupLoadPackage('some.ig', 'current', 'not-loaded', undefined, undefined, undefined, '20200824230227');
      packageCacheMock.getPackageJSONPath.calledWith(pkg.name, 'current').mockReturnValueOnce(pkg.packageJsonPath);
      packageCacheMock.getResourceAtPath.calledWith(pkg.packageJsonPath).mockReturnValueOnce(pkg.resourceAtPath);
      currentBuildClientMock.getCurrentBuildDate.calledWith(pkg.name, undefined).mockReturnValueOnce(pkg.currentBuildDate);
      currentBuildClientMock.downloadCurrentBuild.mockRejectedValue(new Error('error with download current build'));
      const loadPackageFromCacheSpy = jest
        .spyOn(BasePackageLoader.prototype as any, 'loadPackageFromCache')
        .mockImplementation(() => {throw new Error('load pkg from cache error');});

      const result = await loader.loadPackage('some.ig', 'current');
      expect(result).toBe(LoadStatus.FAILED);
      expect(loader.getPackageLoadStatus).toHaveBeenCalledWith('some.ig', 'current');
      expect(loggerSpy.getMessageAtIndex(-1, 'debug')).toBe('Cached package date for some.ig#current (2024-08-24T23:02:27) does not match last build date (2020-08-24T23:02:27)');
      expect(loggerSpy.getMessageAtIndex(-2, 'error')).toBe('Failed to download some.ig#current from current builds');
    ...
    it('should load a specific package with branch from the registry when it is not in the cache', async () => {
      ...
      const result = await loader.loadPackage(pkg.name, pkg.version);
      expect(result).toBe(LoadStatus.LOADED);
      expect(loggerSpy.getLastMessage('info')).toBe('Loaded some.ig#specific-branch$1.2.3 with 5 resources');
    ...
    it('should load a patch versioned package from the registry when it is not in the cache', async () => {
      ...
      const loadPackageFromCacheSpy = jest
        .spyOn(BasePackageLoader.prototype as any, 'loadPackageFromCache')
        .mockReturnValueOnce({ name: pkg.name, version: '1.2.3', resourceCount: 5 });
      
      const result = await loader.loadPackage(pkg.name, pkg.version);
      expect(result).toBe(LoadStatus.LOADED);
      expect(loggerSpy.getLastMessage('info')).toBe('Loaded some.ig#1.2.x with 5 resources'); //TODO: this logs patch, not version number
    ...
    it('should load a latest versioned package from the registry when it is not in the cache', async () => {
      ...
      const loadPackageFromCacheSpy = jest
        .spyOn(BasePackageLoader.prototype as any, 'loadPackageFromCache')
        .mockReturnValueOnce({ name: pkg.name, version: 'latest', resourceCount: 5 });
      
      const result = await loader.loadPackage(pkg.name, pkg.version);
      expect(result).toBe(LoadStatus.LOADED);
      expect(loggerSpy.getLastMessage('info')).toBe('Loaded some.ig#latest with 5 resources'); // TODO: should this log version number
    ...
    it('should log error when a versioned package is unable to be downloaded from the registry', async () => {
      ...
      registryClientMock.download.calledWith(pkg.name, pkg.version).mockImplementation(() => { throw new Error('error with download current build');});
      const loadPackageFromCacheSpy = jest
        .spyOn(BasePackageLoader.prototype as any, 'loadPackageFromCache')
        .mockImplementation(() => {throw new Error('load pkg from cache error');});
      ...
      const result = await loader.loadPackage(pkg.name, pkg.version);
      expect(result).toBe(LoadStatus.FAILED);
      expect(loggerSpy.getMessageAtIndex(-2, 'error')).toBe('Failed to download some.ig#1.2.3 from registry');
    ...
    it('should successfully download package when current version in cache out of date', async () => {
      ...
      const loadPackageFromCacheSpy = jest
        .spyOn(BasePackageLoader.prototype as any, 'loadPackageFromCache')
        .mockReturnValue({ name: pkg.name, version: pkg.version, resourceCount: 5 });
      ...
      expect(loggerSpy.getMessageAtIndex(-1, 'debug')).toBe('Cached package date for some.ig#current (2024-08-24T23:02:27) does not match last build date (2020-08-24T23:02:27)');
      expect(loggerSpy.getMessageAtIndex(-2, 'info')).toBe('Cached package some.ig#current is out of date and will be replaced by the most recent current build.');
      expect(loggerSpy.getLastMessage('info')).toBe('Loaded some.ig#current with 5 resources');
      expect(loadPackageFromCacheSpy).toHaveBeenCalledWith('some.ig', 'current');
      expect(result).toBe(LoadStatus.LOADED);
    });
    ...
    it('should successfully create date for string in messages logged', async () => {
      ...
      expect('2024-08-24T23:02:27'.replace('-', '').replace('-', '').replace(':', '').replace('T', '').replace(':','')).toBe('20240824230227');
    });

    it('should catch error and assume stale version if packageJSONPath was not found', async () => {
      const pkg = setupLoadPackage('some.ig', 'current', 'not-loaded');
      packageCacheMock.getPackageJSONPath.calledWith(pkg.name, 'current').mockReturnValueOnce(undefined);
      currentBuildClientMock.downloadCurrentBuild.calledWith(pkg.name, 'current').mockResolvedValue(pkg.tarball);
      const loadPackageFromCacheSpy = jest
        .spyOn(BasePackageLoader.prototype as any, 'loadPackageFromCache')
        .mockReturnValue({ name: pkg.name, version: pkg.version, resourceCount: 5 });
    ...
    it('should catch error and assume stale version if cachedPackageDate was not found', async () => {
      const pkg = setupLoadPackage('some.ig', 'current', 'not-loaded');
      packageCacheMock.getPackageJSONPath.calledWith(pkg.name, 'current').mockReturnValueOnce(pkg.packageJsonPath);
      packageCacheMock.getResourceAtPath.calledWith(pkg.packageJsonPath).mockReturnValueOnce(undefined);
      currentBuildClientMock.downloadCurrentBuild.calledWith(pkg.name, 'current').mockResolvedValue(pkg.tarball);
      const loadPackageFromCacheSpy = jest
        .spyOn(BasePackageLoader.prototype as any, 'loadPackageFromCache')
        .mockReturnValueOnce({ name: pkg.name, version: pkg.version, resourceCount: 5 });
    ...
    it('should throw error if package is not cached on final load', async () => {
      const humanBeingResourcePath = path.resolve(__dirname, 'fixtures', 'StructureDefinition-human-being-logical-model.json');
      const pkg = setupLoadPackage('human-being-logical-model', '1.0.0', 'not-loaded', undefined, 
        humanBeingResourcePath, undefined, '20240824230227');
      packageCacheMock.isPackageInCache.calledWith(pkg.name, pkg.version).mockReturnValueOnce(true).mockReturnValueOnce(false);

      const result = await loader.loadPackage('human-being-logical-model', '1.0.0');
      expect(result).toBe(LoadStatus.FAILED);
      expect(loggerSpy.getMessageAtIndex(-1, 'error')).toBe('Failed to load human-being-logical-model#1.0.0');
    ...
    it('should use non local package paths to find package with logical flavor', async () => {
      const humanBeingResourcePath = path.resolve(__dirname, 'fixtures', 'StructureDefinition-human-being-logical-model.json');
      const pkg = setupLoadPackage('human-being-logical-model', '1.0.0', 'not-loaded', undefined, 
        humanBeingResourcePath, undefined, '2024-05-24T16:27:17-04:00', 
      ...
      packageCacheMock.getPackagePath.calledWith(pkg.name, pkg.version).mockReturnValue(pkg.packageJsonPath);
      packageCacheMock.getPackageJSONPath.calledWith(pkg.name, pkg.version).mockReturnValue(pkg.packageJsonPath);
      packageCacheMock.getPotentialResourcePaths.calledWith(pkg.name, pkg.version).mockReturnValue([pkg.packageJsonPath]);
      const humanBeingJSON = fs.readJsonSync(pkg.packageJsonPath);
      packageCacheMock.getResourceAtPath.calledWith(pkg.packageJsonPath).mockReturnValue(humanBeingJSON);
      packageDBMock.getPackageStats.calledWith(pkg.name, pkg.version).mockReturnValue({ name: pkg.name, version: pkg.version, resourceCount: 1 });
    ...
    it('should use LOCAL package paths to find package', async () => {
      const humanBeingResourcePath = path.resolve(__dirname, 'fixtures', 'StructureDefinition-human-being-logical-model.json');
      const pkg = setupLoadPackage('LOCAL', 'LOCAL', 'not-loaded', undefined, 
        humanBeingResourcePath, undefined, '2024-05-24T16:27:17-04:00', 
        {resourceType: 'StructureDefinition', id: 'human-being-logical-model'});
      packageCacheMock.isPackageInCache.calledWith(pkg.name, pkg.version).mockReturnValue(true);
      packageCacheMock.getPackagePath.calledWith(pkg.name, pkg.version).mockReturnValue(pkg.packageJsonPath);
    ...
    it('should use non local package paths to find package with profile flavor', async () => {
      const resourceProfileFlavor = path.resolve(__dirname, 'fixtures', 'StructureDefinition-valued-observation.json');
      const pkg = setupLoadPackage('valued-observation', '1.0.0', 'not-loaded', undefined, 
        resourceProfileFlavor, undefined, '2024-05-24T16:27:17-04:00', 
    ...
    it('should use non local package paths to find package with resource flavor', async () => {
      const resourceResourceFlavor = path.resolve(__dirname, 'fixtures', 'StructureDefinition-Condition.json');
      const pkg = setupLoadPackage('Condition', '4.0.1', 'not-loaded', undefined, 
        resourceResourceFlavor, undefined, '2024-05-24T16:27:17-04:00', 
    ...
    it('should use non local package paths to find package with type flavor', async () => {
      const resourceResourceFlavor = path.resolve(__dirname, 'fixtures', 'StructureDefinition-Address.json');
      const pkg = setupLoadPackage('Address', '4.0.1', 'not-loaded', undefined, 
        resourceResourceFlavor, undefined, '2024-05-24T16:27:17-04:00', 
        {resourceType: 'StructureDefinition', id: 'Address'});
      packageCacheMock.isPackageInCache.calledWith(pkg.name, pkg.version).mockReturnValue(true);
      packageCacheMock.getPackagePath.calledWith(pkg.name, pkg.version).mockReturnValue(pkg.packageJsonPath);
      packageCacheMock.getPackageJSONPath.calledWith(pkg.name, pkg.version).mockReturnValue(pkg.packageJsonPath);
      packageDBMock.savePackageInfo.mockImplementation(() => {}); // TODO
      packageCacheMock.getPotentialResourcePaths.calledWith(pkg.name, pkg.version).mockReturnValue([pkg.packageJsonPath]);
      const humanBeingJSON = fs.readJsonSync(pkg.packageJsonPath);
      packageCacheMock.getResourceAtPath.calledWith(pkg.packageJsonPath).mockReturnValue(humanBeingJSON);
      packageDBMock.getPackageStats.calledWith(pkg.name, pkg.version).mockReturnValue({ name: pkg.name, version: pkg.version, resourceCount: 1 });
  ...
  it('should use non local package paths to find package with extension url of imposeprofile', async () => {
    const resourceResourceFlavor = path.resolve(__dirname, 'fixtures', 'StructureDefinition-named-and-gendered-patient.json');
    const pkg = setupLoadPackage('named-and-gendered-patient', '0.1.0', 'not-loaded', undefined, 
      resourceResourceFlavor, undefined, '2024-05-24T16:27:17-04:00', 
      {resourceType: 'StructureDefinition', id: 'valued-observation'});
    packageCacheMock.isPackageInCache.calledWith(pkg.name, pkg.version).mockReturnValue(true);
    packageCacheMock.getPackagePath.calledWith(pkg.name, pkg.version).mockReturnValue(pkg.packageJsonPath);
    ...
    expect(packageDBMock.saveResourceInfo).toHaveBeenCalledWith(
      {
        ...
        sdAbstract: false,
        sdFlavor: 'Profile',
        sdImposeProfiles: ['http://example.org/impose/StructureDefinition/named-patient', 'http://example.org/impose/StructureDefinition/gendered-patient'],
  ...
  it('should use non local package paths to find package with package json path that leads to non-existing resource', async () => {
    ...
    packageCacheMock.getPackageJSONPath.calledWith(pkg.name, pkg.version).mockReturnValue(pkg.packageJsonPath);
    packageCacheMock.getPotentialResourcePaths.calledWith(pkg.name, pkg.version).mockReturnValue([pkg.packageJsonPath]);
    packageCacheMock.getResourceAtPath.calledWith(pkg.packageJsonPath).mockImplementation(() => {throw new Error('load resource at json path error');});
    packageDBMock.getPackageStats.calledWith(pkg.name, pkg.version).mockImplementation(() => {throw new Error('get package stats error');});
  ...
  it('should use non local package paths to find package with extension flavor', async () => {
    const resourceResourceFlavor = path.resolve(__dirname, 'fixtures', 'StructureDefinition-patient-birthPlace.json');
    const pkg = setupLoadPackage('patient-birthPlace', '4.0.1', 'not-loaded', undefined, 
      resourceResourceFlavor, undefined, '2024-05-24T16:27:17-04:00', 
  ...
  describe('#findPackageJSONs', () => {
    it ('should return package json array', () => {
      const name = 'some.ig';
      loader.findPackageJSONs = jest.fn().mockReturnValueOnce([{'resource' : 'json/path/name', 'date': '20240824230227', 'resourceType': 'resourceTypeName'}]);
  ...
  describe('#findPackageJSON', () => {
    it ('should return package json array', () => {
      const name = 'some.ig';
      const version = '1.2.3';
      loader.findPackageJSON = jest.fn().mockReturnValueOnce({'resource' : 'json/path/name', 'date': '20240824230227', 'resourceType': 'resourceTypeName'});
  ...
  describe('#findResourceInfos', () => {
    it ('should return resource array', () => {
      const name = 'some.ig';
      loader.findResourceInfos = jest.fn().mockReturnValueOnce([{'resource' : 'json/path/name', 'date': '20240824230227', 'resourceType': 'resourceTypeName'}]);
  ...
  describe('#findResourceInfo', () => {
    it ('should return resource info', () => {
      const name = 'some.ig';
      loader.findResourceInfo = jest.fn().mockReturnValueOnce({'resource' : 'json/path/name', 'date': '20240824230227', 'resourceType': 'resourceTypeName'});
  ...
  describe('#findResourceJSONs', () => {
    it ('should return resource json array', () => {
      const name = 'some.ig';
      loader.findResourceJSONs = jest.fn().mockReturnValueOnce([{'resource' : 'json/path/name', 'date': '20240824230227', 'resourceType': 'resourceTypeName'}]);
  ...
  describe('#findResourceJSON', () => {
    it ('should return resource json', () => {
      const name = 'some.ig';
      loader.findResourceJSON = jest.fn().mockReturnValueOnce({'resource' : 'json/path/name', 'date': '20240824230227', 'resourceType': 'resourceTypeName'});
